  ...
  public partial class HelpForm : Form {
    ...
    public HelpForm() {
      ...
      var richTextBox = new RichTextBox() {
        ...
        Text = "版本号：" + version +
         "\n更新说明：\n" +
         "（1）替换WinRing0为PawnIO，以避免Microsoft的拦截。使用前必须先安装PawnIO：https://pawnio.eu/；\n" +
         "（2）修复重启后可能无法恢复最大风扇转速的问题。\n\n" +
         ...
         "（5）由于其锁定功耗的特性，OXH会短暂地设置狂暴+CTGP开+DB开进行自动解锁。同样地，请注意不要在CPU高负载时切换解锁版本，否则也会导致解锁后只有CTGP开+DB关的功耗。系统重启后解锁会失效造成功耗限制为基础功耗，需要由软件自动完成启用再禁用驱动的操作恢复解锁状态，因此使用解锁功能最好打开OXH开机自启；\n" +
         ...
         "（2）若选择“切换浮窗显示”，Omen键绑定的事件为启动OXH，通过传递特定参数使OXH获得指令并切换浮窗显示；\n" +
         ...
         "（4）设置开机自启后，程序会自动创建任务计划程序实现开机自启，如果有自己设定过则应删除。同时，旧版OXH使用的自启方式也会在这一步被自动清除；\n" +
